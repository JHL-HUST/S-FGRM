...
tf.flags.DEFINE_integer('number', 10, 'the number of images for variance correction')

tf.flags.DEFINE_float('portion', 1.5, 'the bound for variance correction.')
...
def rescaling_method(noise):
    sign = tf.sign(noise)
    affine = -tf.math.log((tf.abs(noise)))/tf.math.log(2.)
    affine_max = tf.reduce_max(affine,axis=[1,2],keep_dims=True)
    affine = tf.ceil(affine_max-affine+1e-9)
    weight = 2. * tf.log1p(affine/affine_max)
    return tf.multiply(weight,sign)
...
def batch_grad(x, one_hot, i, max_iter, alpha, grad):
    ...
    with tf.device('/gpu:2'):
        with slim.arg_scope(resnet_v2.resnet_arg_scope()):
            logits_res_v2, end_points_res_v2 = resnet_v2.resnet_v2_101(
                input_diversity(x_res), num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)
        logits_v3 = (logits_inc_v3 + logits_inc_v4 + logits_inc_res + logits_res_v2) / 4.    
    ...
    i = tf.add(i, 1)
    return x_neighbor, one_hot, i, max_iter, alpha, grad
...
def graph(x, y, i, x_max, x_min, grad, variance):
    ...
    pred = tf.argmax(end_points_inc_v3['Predictions'] + end_points_inc_v4['Predictions'] + end_points_inc_res['Predictions'] + \
                        end_points_res_v2['predictions'], 1)
    ...
    with tf.device('/gpu:0'):
        ...
        _, _, _, _, _, global_grad = tf.while_loop(grad_finish, batch_grad, [x, one_hot, iter, max_iter, eps*FLAGS.portion, tf.zeros_like(new_grad)])


    noise = (global_grad + new_grad) / (1. * (FLAGS.number + 1))
    ...
    x = x + alpha * rescaling_method(noise)
